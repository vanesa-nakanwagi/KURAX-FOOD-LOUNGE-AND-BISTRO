...
export default function AccountantDashboard() {
 const [searchTerm, setSearchTerm] = useState("");
  ...
  // 2. Dynamic System Totals calculation
  // This filter ensures ONLY 'ACTIVE' orders are counted in your cards
  ...
  // 1. Define systemTotals ONCE here
  const systemTotals = { cash: 1250000, momo: 2100000, card: 1500000 };
 ...
 const generateWhatsAppReport = () => {
    ...
    // 3. Calculate Expected Handover (Cash minus what was spent)
    const netExpectedCash = systemTotals.cash - pettyCashTotal;

    // 4. Build the dynamic string
    const report = `
...
----------------------------------
_Report generated by Kurax Finance System_`;
  ...
  return (
    <div className="space-y-8 p-4 md:p-8 bg-[#0a0a0a] min-h-screen text-slate-200">
      
      {/* HEADER */}
      <div className="flex flex-wrap justify-between items-end gap-4">
        <div>
          <h2 className="text-2xl font-black italic uppercase text-white tracking-tighter">Finance Audit</h2>
          <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-[0.2em]">Kurax Food Lounge</p>
        </div>
      ...
      {/* 1. THE COUNTER (LIVE TILL STATUS) */}
<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
  ...
  <AccountantStatCard 
    label="Expected Cash (After Petty)" 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-white/5">
              <AuditRow id="#9021" staff="Alex" amount={45000} method="CASH" time="14:20" />
              <AuditRow id="#9020" staff="Sarah" amount={120000} method="MOMO" time="14:15" />
            </tbody>
          </table>
        </div>
      </div>
...
function AuditRow({ order, onVoid }) {
  ...
  return (
    <tr className={`hover:bg-white/5 transition-all ${isVoided ? 'opacity-25 grayscale' : ''}`}>
      <td className="p-6">
        <div className="flex flex-col">
          <span className="text-white italic font-bold">{order.id}</span>
          {isVoided && <span className="text-[7px] text-rose-500 font-black uppercase">Voided: {order.voidReason}</span>}
        </div>
      </td>
      <td className="p-6 text-zinc-500 uppercase text-xs font-bold">{order.staff}</td>
      <td className={`p-6 font-black text-sm ${isVoided ? 'line-through text-zinc-600' : ''}`}>
        UGX {order.amount.toLocaleString()}
      </td>
      <td className="p-6">
        <span className="text-[10px] font-black px-2 py-1 rounded border border-white/10 uppercase">
          {order.method}
        </span>
      </td>

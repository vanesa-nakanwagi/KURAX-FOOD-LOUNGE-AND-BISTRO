...
export default function Menu() {
  ...
  // Listen for custom search event from TopSection
  useEffect(() => {
  ...
  return (
    /*
      ...
      1. FIXED HEADER ZONE  — TopSection (fixed to viewport via its own CSS) +
                              the category tabs bar which we also fix right below it.
                              Neither of these scrolls.

      2. SCROLLABLE CONTENT — A single <div> with overflow-y-auto that holds the
                              page header, the menu grid, etc.
                              Only THIS div scrolls, so window.scrollY stays 0
                              and the fixed TopSection never disappears.

      The category bar sits in its own `fixed` strip immediately below TopSection.
      A matching spacer `pt-` on the scrollable content compensates for both bars

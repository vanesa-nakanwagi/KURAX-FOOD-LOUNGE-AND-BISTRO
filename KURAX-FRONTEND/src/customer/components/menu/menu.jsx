...
function MenuCard({ item, onOrder, isNew }) {
  ...
  return (
    <div className="group font-['Outfit'] relative bg-white dark:bg-[#111111] rounded-[2rem] overflow-hidden flex flex-col transition-all duration-500 hover:-translate-y-2 border border-zinc-200 dark:border-zinc-800 hover:border-yellow-500/30 shadow-sm hover:shadow-xl">
      ...
      {/* IMAGE CONTAINER — taller, full coverage */}
      <div className="relative h-56 sm:h-64 md:h-72 overflow-hidden bg-zinc-100 dark:bg-[#1a1a1a] shrink-0">
        {!imgLoaded && (
        ...
        />
        {/* Gradient overlay — only at bottom so most of image stays visible */}
      ...
      <div className="p-5 md:p-6 flex flex-col gap-3 flex-1">
        ...
        <div className="flex items-start justify-between gap-3">
          <h4 className="text-lg md:text-xl font-semibold text-zinc-900 dark:text-white leading-snug flex-1">
            {item.name}
          </h4>
          <div className="text-right shrink-0">
            <span className="text-base md:text-lg text-yellow-600 dark:text-yellow-400 font-bold leading-none">
              {Number(item.price).toLocaleString()}
            </span>
            <div className="text-[8px] tracking-widest text-zinc-400 uppercase mt-0.5">UGX</div>
          </div>
        </div>

        {/* Description */}
        <p className="text-[12px] md:text-[13px] text-zinc-500 dark:text-zinc-400 line-clamp-2 leading-relaxed font-light flex-1">
        ...
        <button
          ...
          className="w-full py-3.5 rounded-xl bg-yellow-400 hover:bg-black text-black hover:text-yellow-400 text-[10px] md:text-xs font-bold uppercase tracking-[0.15em] flex items-center justify-center gap-2 transition-all duration-300 active:scale-95 mt-1"
        >
  ...
  return (
    <div className="font-['Outfit'] bg-[#F9F9F7] dark:bg-[#080808] text-zinc-900 dark:text-white min-h-screen transition-colors duration-500">
          ...
          <div className="flex items-center gap-3 pl-4 h-fit text-right shrink-0">
            <div>
              ...
              <p className="text-lg md:text-2xl font-semibold leading-none">
                ...
                <span className="text-[10px] font-normal opacity-60">Dishes</span>
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-5 md:px-12 pb-24 md:pb-32">
        {loading ? (
          // Skeleton — 3 cols max, taller cards
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="rounded-[2rem] overflow-hidden bg-zinc-100 dark:bg-zinc-900 animate-pulse"
                style={{ height: "420px" }}
              >
                {/* Skeleton image area */}
          ...
          <div className="py-20 text-center border-2 border-dashed border-zinc-100 dark:border-zinc-800 rounded-[2.5rem]">
            <p className="italic text-zinc-400">
              Our chefs are preparing something new for this category...
            </p>
          </div>
        ) : (
          // 3 cols max so each card is wide enough for the image to breathe
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">

...
// 3. EDIT TRIGGER METHOD
const handleEdit = (event) => {
  setEditingEvent(event);
  
  // Date input needs YYYY-MM-DD format exactly
  ...
  setFormVisible(true);
  // Smooth scroll back to the top where the form is
      ...
      <div id="events-content" className="flex-1 flex flex-col overflow-y-auto">
        <main className="p-4 md:p-8 pt-20 lg:pt-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            {!formVisible && (
              <button onClick={() => setFormVisible(true)} className="flex items-center justify-center gap-2 px-6 py-3 bg-yellow-500 text-black font-bold rounded-xl hover:bg-yellow-600 transition shadow-lg active:scale-95">
          ...
          {formVisible && (
            <div className="mb-10 bg-zinc-900 border border-slate-800 p-5 md:p-8 rounded-2xl shadow-xl animate-in fade-in duration-300">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-1.5">
                    ...
                    <input type="text" name="name" value={formData.name} onChange={handleChange} required className="w-full bg-zinc-800 border border-slate-700 p-3 rounded-xl focus:border-yellow-500 outline-none text-white" />
                  </div>
                  <div className="space-y-1.5">
...
<div className="space-y-1.5">
  ...
  />
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-2 p-2 bg-zinc-800 border border-slate-700 rounded-xl min-h-[50px] focus-within:border-yellow-500 transition-colors">
  ...
  onKeyDown={(e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      e.stopPropagation(); // Stops the "Enter" from reaching the form submit
      addTag(e);
    }
  }} 
  placeholder="Add tag..." 
  className="bg-transparent outline-none text-sm text-white flex-1 p-1" 
/>
                  </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                  <input type="date" name="date" value={formData.date} onChange={handleChange} required className="bg-zinc-800 border border-slate-700 p-3 rounded-xl text-sm text-white outline-none focus:border-yellow-500" />
                  <input type="time" name="time" value={formData.time} onChange={handleChange} required className="bg-zinc-800 border border-slate-700 p-3 rounded-xl text-sm text-white outline-none focus:border-yellow-500" />
                  <input type="file" accept="image/*" onChange={handleFileChange} className="bg-zinc-800 border border-slate-700 p-2.5 rounded-xl text-xs text-slate-400 file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:bg-yellow-500 file:text-black file:font-bold" />
                </div>

                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 mt-4 border-t border-slate-800/50">
                  <div className="flex items-center gap-3">
                    ...
                    <label htmlFor="published" className="text-xs font-bold text-slate-400 uppercase tracking-wider cursor-pointer hover:text-white">Make this event live</label>
                  </div>
                  <div className="flex items-center gap-3 w-full sm:w-auto">
                    <button type="button" onClick={resetForm} disabled={isSaving} className="flex-1 sm:flex-none px-6 py-3 text-slate-400 font-bold hover:text-white text-sm">Cancel</button>
                    <button type="submit" disabled={isSaving} className={`flex-1 sm:flex-none px-8 py-3 rounded-xl text-black font-bold transition-all text-sm shadow-lg flex items-center justify-center gap-2 ${
                      isSaving ? 'bg-yellow-600 opacity-70 cursor-not-allowed' : 'bg-yellow-500 hover:bg-yellow-600 active:scale-95'
                    }`}>
            ...
            {events.map(event => (
              <div key={event.id} className="group bg-zinc-900 border border-slate-800 rounded-2xl overflow-hidden hover:border-yellow-500/30 transition-all flex flex-col shadow-lg shadow-black/50">
                <div className="h-44 md:h-48 bg-zinc-800 flex items-center justify-center relative flex-shrink-0">
                  ...
                  {/* TAGS ON TOP LEFT OF CARD IMAGE */}
                  <div className="absolute top-3 left-3 flex flex-wrap gap-1.5 max-w-[75%]">
                    {getParsedTags(event.tags).map((tag, i) => (
                      <span key={i} className="px-2.5 py-1 bg-yellow-400/90 backdrop-blur-sm text-black text-[9px] font-black uppercase tracking-widest rounded-full shadow-lg">
                        {tag}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="absolute top-3 right-3">
                    <span className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black uppercase ${event.published ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-zinc-800/80 text-slate-500 border border-slate-700'}`}>
  ...
  <p className="text-[12px] text-slate-400 mb-4 line-clamp-2  leading-relaxed">
    {event.description || "No description provided."}
                  ...
                  <div className="flex gap-2.5 pt-4 border-t border-slate-800 mt-auto">
                    <button onClick={() => handleEdit(event)} className="flex-1 flex items-center justify-center gap-2 bg-zinc-800 hover:bg-zinc-700 py-2.5 rounded-xl font-bold text-slate-200 active:scale-95 transition">
                      <Edit2 className="w-3.5 h-3.5" /> Edit
                    </button>
                    <button onClick={() => handleDelete(event.id)} className="px-4 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-xl active:scale-95 transition">

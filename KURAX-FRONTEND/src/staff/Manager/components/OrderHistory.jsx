...
export default function OrderHistory() {
  // Pull currentUser to identify who is looking at the history
  ...
  const currentStaffName = currentUser?.name || "Staff"; 
  const currentStaffId = currentUser?.id;
  const today = new Date().toISOString().split('T')[0];

  // 1. Filter orders specifically for the logged-in Manager/Staff
  const myDailyOrders = useMemo(() => {
    return orders.filter(order => {
      const orderDate = new Date(order.timestamp).toISOString().split('T')[0];
      // Match by staffId (preferred) or name, and ensure it's from today
  ...
  // 2. Grouping Logic for the current user's tables
  const groupedTableOrders = useMemo(() => {
    return myDailyOrders.reduce((acc, order) => {
  ...
  const progressPercent = Math.min((myDailyOrders.length / DAILY_GOAL) * 100, 100);

  const totals = myDailyOrders.reduce((acc, order) => {
    if (order.isPaid) {
  ...
  return (
    <div className={`p-4 md:p-8 min-h-screen font-[Outfit] pb-24 transition-colors duration-300 ${theme === 'dark' ? 'bg-black text-white' : 'bg-zinc-50 text-zinc-900'}`}>
      ...
      <MomoPaymentModal 
        ...
        orderId={selectedOrder?.orderIds[0]}
        onConfirm={() => {
          ...
          <div className="bg-zinc-900 border border-white/10 w-full max-w-2xl rounded-[3rem] overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-4">
              {currentOpenOrder.items
                .filter(item => item && item.name && item.status !== "VOIDED")
                .map((item, idx) => {
                    ...
                    <div key={itemKey} className="bg-black/40 border border-white/5 rounded-[2rem] p-6 flex justify-between items-center">
                      ...
                      {item.voidRequested ? (
                        ...
                        <button 
                          ...
                          className="text-[9px] font-black uppercase text-rose-500 border border-rose-500/30 px-4 py-2 rounded-full hover:bg-rose-500 transition-all"
                        >Request Void</button>
                      )}
                    </div>
                  );
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-tighter italic">Personal Dashboard</h1>
            <p className="text-zinc-500 text-xs font-bold uppercase tracking-tight">Viewing your orders only</p>
          </div>
          <div className={`inline-flex items-center gap-3 px-4 py-2 rounded-2xl border ${theme === 'dark' ? 'bg-zinc-900 border-white/10' : 'bg-white border-black/5 shadow-sm'}`}>
            ...
            <h3 className="text-[11px] font-black uppercase tracking-tight text-yellow-500">{currentStaffName}</h3>
          </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-6">
        <div className={`p-4 rounded-2xl border ${theme === 'dark' ? 'bg-zinc-900 border-slate-800' : 'bg-white border-black/5'}`}>
          ...
          <h3 className="text-lg font-black">{myDailyOrders.length} <span className="text-[10px] opacity-30">/ {DAILY_GOAL}</span></h3>
          ...
          filteredOrders.map((order) => (
            <div key={order.tableName} onClick={() => setSelectedOrderId(order.orderIds[0])} className={`p-4 rounded-[2rem] flex items-center justify-between cursor-pointer transition-all border-2 ${theme === 'dark' ? `bg-zinc-900 border-white/5` : `bg-white border-black/5 shadow-sm`}`}>
              ...
              <div className="flex items-center gap-2">
                {order.status === "Ready" && (
                  <button onClick={(e) => markAsServed(e, order.orderIds)} className="bg-emerald-500 text-black px-4 py-2 rounded-xl text-[9px] font-black uppercase italic shadow-lg active:scale-95 transition-transform">Serve</button>
                )}
                {!order.isPaid ? (
                  <button onClick={(e) => handlePaymentTrigger(e, order)} className="bg-yellow-500 text-black px-4 py-2 rounded-xl text-[9px] font-black uppercase italic shadow-lg active:scale-95 transition-transform flex items-center gap-1.5"><Banknote size={12} /> Pay</button>
                ) : (
...
// --- SUB-COMPONENTS REMAIN THE SAME AS PREVIOUS ---

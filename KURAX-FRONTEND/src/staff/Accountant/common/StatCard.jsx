...
export default function StatCard({ icon, label, value, color, gradient, note, trend, isCompact = true }) {
  ...
  return (
    <div className={`group relative overflow-hidden rounded-2xl ${cardBg} p-5 border border-gray-200 shadow-sm hover:shadow-md hover:border-yellow-300 transition-all duration-300 hover:scale-[1.02]`}>
      <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-yellow-500/5 to-transparent rounded-full -mr-16 -mt-16 group-hover:scale-150 transition-transform duration-700" />
      <div className="absolute bottom-0 left-0 w-24 h-24 bg-gradient-to-tr from-gray-100 to-transparent rounded-full -ml-12 -mb-12 group-hover:scale-150 transition-transform duration-700" />
      
      <div className="relative z-10">
        <div className={`p-3 w-fit rounded-xl bg-gray-100 border border-gray-200 ${color} group-hover:scale-110 transition-all duration-300 group-hover:border-yellow-300`}>
          {IconComponent}
        </div>
        ...
        <div className="mt-4 mb-2">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[9px] font-black uppercase text-gray-500 tracking-[0.2em] group-hover:text-yellow-600 transition-colors">
              {label}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-baseline gap-2 flex-wrap">
          <h3 className={`text-2xl sm:text-3xl font-black tracking-tighter group-hover:tracking-tight transition-all ${color || "text-gray-900"}`}>
            {formattedValue}
          </h3>
          {note && (

      ...
      <div className={`w-full sm:max-w-md sm:mx-4 rounded-t-[2rem] sm:rounded-[2rem] p-5 sm:p-10 shadow-2xl border
        ...
        <div className="flex justify-between items-center mb-5">
          ...
          <button onClick={onClose}
            className={`p-2 rounded-xl ${dark ? "hover:bg-white/5 text-zinc-500" : "hover:bg-zinc-100 text-zinc-400"}`}>
            <X size={15} />
          </button>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            <input type={showPin ? "text" : "password"} placeholder="ACCESS PIN (4 digits)"
              value={form.pin} maxLength={4}
            ...
            <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-0.5">
              <button type="button" onClick={genPin}
                className={`p-2 ${t.subtext} hover:text-yellow-500 transition-colors`}>
                <RefreshCcw size={14} />
              </button>
          ...
          {!initialData && (
            <p className={`text-[9px] italic px-1 ${t.subtext}`}>
          ...
          <div className="flex gap-3 pt-4">
            <button onClick={onClose}
              className={`flex-1 py-3.5 font-black uppercase italic text-xs ${t.subtext}`}>
              Cancel
            </button>
            <button type="button" disabled={isSubmitting} onClick={submit}
              className={`flex-[2] py-3.5 rounded-2xl font-black uppercase italic text-sm transition-all active:scale-95
                ${isSubmitting ? "bg-zinc-800 text-zinc-500 cursor-not-allowed" : "bg-yellow-500 text-black"}`}>

...
export default function CashierDashboard() {
  ...
  // ── Today date (refreshes at midnight) ─────────────────────────────────────
  ...
  // ── TODAY TOTALS (FIXED — single unified useMemo) ───────────────────────────
  const todayTotals = useMemo(() => {
    const base = { cash: 0, card: 0, momo_mtn: 0, momo_airtel: 0, credit: 0, gross: 0 };

    // Step 1: confirmed orders from cashier history
    history.forEach(row => {
      ...
      switch (row.method) {
        ...
        case "Credit":      base.credit      += amt; break; // excluded from gross
        default: break;
      }
    });

    // Step 2: credit settlements — route into correct gross bucket by settle_method
    credits.forEach(c => {
      const isSettled =
      ...
      const settledDate = toLocalDateStr(new Date(c.paid_at || c.updated_at || c.created_at));
      if (settledDate !== today) return;

      const amt = Number(c.amount_paid || c.amount || 0);
      if (amt <= 0) return;

      const method = c.settle_method || "Cash";
      switch (method) {
        case "Cash":        base.cash        += amt; base.gross += amt; break;
        case "Card":        base.card        += amt; base.gross += amt; break;
        case "Momo-MTN":    base.momo_mtn    += amt; base.gross += amt; break;
      ...
      // NOTE: We do NOT subtract from base.credit here — the settlement is a
      // new cash inflow; the original credit entry was already forwarded.
    });
  ...
  const handleSettleCredit = useCallback(async (creditId, settleData) => {
    try {
      ...
      console.error("Settle credit failed:", e);
    }
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="flex items-center justify-between px-6 py-4 bg-[#0c0c0c] border-b border-white/5 sticky top-0 z-50">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h2 className="text-2xl md:text-3xl font-bold text-white tracking-tight">
                Welcome back,{" "}
                <span className="text-yellow-400 capitalize">{cashierName}</span>
              </h2>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            {forwardedQueue.length > 0 && (
              <div className="flex items-center gap-2 px-3 py-2 bg-purple-500/20 border border-purple-500/30 rounded-xl">
                ...
                <span className="text-[10px] font-black text-purple-400 uppercase tracking-widest">
                  {forwardedQueue.length} Awaiting Manager
                </span>
              </div>
            )}
            {deliveryBadge > 0 && (
            ...
            <button
              onClick={fetchAll}
              className="p-2 bg-zinc-900 rounded-xl text-zinc-400 hover:text-white transition-all"
            >
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-top-4 duration-700">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                ...
                <div className="bg-yellow-500 p-5 rounded-[2rem] border border-yellow-400 flex flex-col gap-2 col-span-2 md:col-span-1">
                  <div className="p-2.5 w-fit rounded-xl bg-black/20 text-black">
                    <TrendingUp size={18} />
                  </div>
                  <div>
              ...
              {unpaidCredits.length > 0 && (
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="px-2 py-0.5 bg-purple-500 text-white text-[9px] rounded-full font-black">
                      {unpaidCredits.length} unpaid
                    </span>
                  </div>
                  <div className="space-y-3">
                    {credits.map(credit => (
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {normalQueue.map(order => (
                      <div
                        key={order.id}
                        className="bg-zinc-900/20 border border-white/5 rounded-[2rem] p-4 flex items-center justify-between gap-4 flex-wrap"
                      >
                        <div>
                          <p className="text-sm font-black text-white uppercase italic">
                            {order.table_name || `Order #${order.id}`}
                          </p>
            ...
            {!rejecting ? (
              <div className="flex flex-col gap-3">
                {isCredit ? (
                  <>
                    <button
                      onClick={() => handleRequestApproval(processingOrder)}
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            <span className="text-[9px] bg-purple-500/10 border border-purple-500/20 text-purple-400 px-2 py-0.5 rounded-lg font-black uppercase">
              Credit
            </span>
            {order.credit_name && (
              <span className="text-[9px] text-zinc-500 font-bold">{order.credit_name}</span>
            )}
          </div>
    ...
    >
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 flex-wrap mb-2">
          <span className="font-black text-white uppercase italic tracking-tighter text-base">
          ...
          {credit.paid ? (
            ...
            <span className="px-2 py-0.5 rounded-lg bg-purple-500/10 border border-purple-500/20 text-purple-400 text-[9px] font-black uppercase animate-pulse">
              Unpaid
            </span>
          )}
        </div>
      ...
      <div className="bg-white text-black w-full max-w-sm rounded-[3rem] p-12 font-mono shadow-2xl relative overflow-hidden">
        ...
        <div className="space-y-4 border-y border-dashed border-zinc-200 py-8 mb-8">
          ...
          <div className="flex justify-between text-sm">
            <span>CASH:</span>
            <span className="font-bold">UGX {data?.cash?.toLocaleString()}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span>MOMO:</span>
            <span className="font-bold">UGX {data?.momo?.toLocaleString()}</span>
          </div>
        </div>
        <div className="flex flex-col gap-3">
          <button
          ...
          <button onClick={onClose} className="w-full py-4 text-zinc-400 font-bold uppercase text-[10px]">
            Close Window
          </button>
        </div>
      </div>

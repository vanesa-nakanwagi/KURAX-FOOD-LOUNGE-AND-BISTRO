...
import {
  ...
  User, Mail, Phone, Calendar, Star, Zap, BarChart3
} from "lucide-react";
...
export default function StaffTargets() {
  ...
  const getProgressWidth = (value, max = 5000000) => {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-gradient-to-br from-gray-50 to-gray-100 font-[Outfit]">
      ...
      <div className="shrink-0 border-b border-gray-200 bg-white/80 backdrop-blur-sm px-4 pt-6 pb-4 space-y-3 shadow-sm sticky top-0 z-10">
        ...
        <div>
          ...
          <div className="mt-2">
            <p className="text-[10px] text-gray-500 leading-relaxed">
              Select any team member to view and adjust their monthly revenue and daily order targets. 
              Changes take effect immediately and staff members will see updated progress in real-time.
            </p>
          </div>
        </div>
        ...
        {/* Pill row with gradient fade effects */}
      ...
      <div className="flex-1 overflow-y-auto p-6 pt-8">
        {selected ? (
          <div className={`max-w-3xl mx-auto space-y-6 transition-all duration-500 ${animateIn ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
                ...
                <div className="p-5 space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400">Goal for the current month</p>
                      </div>
                    </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-[10px] font-bold">
                      ...
                      <span className="text-yellow-600">{Math.min(100, Math.round(getProgressWidth(selected.monthly_income_target || 0)))}%</span>
                    </div>
                    <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-yellow-400 to-yellow-600 rounded-full transition-all duration-700"
                        style={{ width: `${getProgressWidth(selected.monthly_income_target || 0)}%` }}
                      />
                    </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <InsightCard 
                  ...
                  value={selected.monthly_income_target > 2000000 ? "Top Performer" : "Rising Star"} 
                  accent="yellow"
                  metric={`${Math.round(getProgressWidth(selected.monthly_income_target || 0))}% to target`}
                />
                <InsightCard 
                  icon={<Target size={16} className="text-blue-600"/>} 
                  label="Projected Bonus" 
                  value={selected.monthly_income_target > 3000000 ? "UGX 350,000" : "UGX 150,000"} 
                  accent="blue"
                  metric={`${selected.monthly_income_target > 3000000 ? "Exceeding" : "Meeting"} expectations`}
                />
                <InsightCard 
            ...
            {/* Achievement Badge */}
            {(selected.monthly_income_target > 4000000 || selected.daily_order_target > 80) && (

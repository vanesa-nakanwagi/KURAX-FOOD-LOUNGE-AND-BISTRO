...
export default function ShiftReportModal({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        // ── 1. Check DB: does this supervisor have orders today? ─────────────
        const previewRes = await fetch(
          `${API_URL}/api/waiter/supervisor-shift-preview?staff_id=${staffId}&staff_name=${encodeURIComponent(supervisorName)}&date=${today}`
        );
        if (previewRes.ok && active) {
          ...
          console.error("[ShiftModal] supervisor-shift-preview failed:", previewRes.status, await previewRes.text().catch(() => ""));
        }

        // ── 2. Credit decisions (always fetch — independent of orders) ────
        const creditRes = await fetch(
          `${API_URL}/api/waiter/supervisor-credit-stats?supervisor_name=${encodeURIComponent(supervisorName)}&date=${today}`
        );
    ...
    load();
    return () => { active = false; };
  }, [isOpen, staffId, supervisorName, today]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={`text-[9px] font-bold uppercase tracking-widest ${isDark ? "text-zinc-500" : "text-zinc-400"}`}>
                {supervisorName} · Supervisor · {today}
              </p>
            </div>
          </div>

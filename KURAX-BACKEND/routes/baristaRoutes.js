 ...
 * Endpoints
 * ─────────────────────────────────────────────────────────────────────────────
 *  POST   /tickets                      upsert ticket when order enters barista station
 *  GET    /tickets?date=YYYY-MM-DD      fetch tickets for a date (default today Kampala)
 *  GET    /tickets/summary?date=…       daily report — count, cups, barista breakdown
 *  PATCH  /tickets/:id/status           Pending → Preparing → Ready
 *  PATCH  /tickets/:id/assign-barista   assign a barista to one drink item
...
// ─────────────────────────────────────────────────────────────────────────────
// 1.  POST /api/barista/tickets  — upsert
// ─────────────────────────────────────────────────────────────────────────────
...
router.get('/tickets/summary', async (req, res) => {
  ...
  try {
    ...
    const baristaRes = await pool.query(
      `SELECT
         ba.assigned_to              AS barista,
         COUNT(*)                    AS drinks_made,
         MIN(ba.assigned_at)         AS first_at,
         MAX(ba.assigned_at)         AS last_at
       FROM barista_assignments ba
       JOIN barista_tickets bt ON ba.ticket_id = bt.id
    ...
    res.json({ date, totals: totalsRes.rows[0], baristas: baristaRes.rows, tickets: ticketsRes.rows });
  } catch (err) {
    console.error('Barista summary error:', err.message);
    res.status(500).json({ error: err.message });
  }
});

// ─────────────────────────────────────────────────────────────────────────────
// 2b. GET /api/barista/tickets?date=…
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// 3.  PATCH /api/barista/tickets/:id/status
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// 4.  PATCH /api/barista/tickets/:id/assign-barista
// ─────────────────────────────────────────────────────────────────────────────
router.patch('/tickets/:id/assign-barista', async (req, res) => {
  const { id } = req.params;
  ...
  try {
    const ticketResult = await pool.query(
      `UPDATE barista_tickets SET items = $1, updated_at = NOW() WHERE id = $2 RETURNING *`,
    ...
    await pool.query(
      `INSERT INTO barista_assignments (order_id, ticket_id, item_name, assigned_to, assigned_at, assigned_by)
       VALUES ($1, $2, $3, $4, $5, $6)`,
